...
test("assets schema no longer persists sha256 or its index", () => {
  const libDir = mkdtempSync(path.join(tmpdir(), "tags-market-store-"));
  const store = new AssetStore(libDir);
  try {
    const cols = store.db.prepare("PRAGMA table_info(assets)").all().map((row) => row.name);
    assert.equal(cols.includes("sha256"), false, "assets 表不应再有 sha256 列");
    const indexes = store.db.prepare("PRAGMA index_list(assets)").all().map((row) => row.name);
    assert.equal(indexes.includes("idx_assets_sha"), false, "不应再有 idx_assets_sha 索引");
  } finally {
    store.close();
    rmSync(libDir, { recursive: true, force: true });
  }

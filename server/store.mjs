...
// 每次生成都是独立 asset（新 id + 新文件 + 新行），不按内容去重。
...
export class AssetStore {
  ...
  // 保存图片：buffer 落盘 + 元数据入库。返回 { id, file_path }。
  // 每次调用都落一个独立 asset（新 id + 新文件 + 新行），不做内容去重：
  ...
  saveImage(buffer, { mime = "image/png", width = null, height = null, source = "novelai", job_id = null, prompt = null, negative_prompt = null, parameters_json = null, batch_id = null, batch_index = null, batch_total = null, correlation_id = null }) {
    ...
    this.db
      .prepare(
        `INSERT INTO assets (id, file_path, mime, byte_size, width, height, created_at, source, job_id, prompt, negative_prompt, parameters_json, batch_id, batch_index, batch_total, correlation_id)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
      )
      .run(id, relPath, mime, buffer.length, width, height, Date.now(), source, job_id, prompt, negative_prompt, parameters_json ?? null, batch_id, batch_index, batch_total, correlation_id);

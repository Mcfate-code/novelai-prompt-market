...
export const STATUS_SYMBOLS = { filled: "✓", filled_by_auto_preset: "✓", satisfied_by_alternative: "✓", partial: "◐", empty: "○" };
export const STATUS_LABELS = { filled: "已完成", filled_by_auto_preset: "自动完成", satisfied_by_alternative: "由替代项满足", partial: "部分完成", empty: "未设置" };
...
const BASE_SYNTHETIC = {
  base_subject_count: { id: "base_subject_count", label: "主体 / 人数", zh: "主体/人数", target: "base", section: "character", children: [] },
  quality: { id: "quality", label: "画质", zh: "画质", target: "base", section: "quality", children: [], displayOnly: true },
};
...
const COMPLETE = new Set(["filled", "filled_by_auto_preset", "satisfied_by_alternative"]);
...
export function workspaceTabs(doc) { return [{ key: "base", label: "基础", index: null }, ...(doc?.characters || []).map((character, index) => ({ key: `char:${index}`, label: localizedTargetLabel(doc, `char:${index}`), index }))]; }
...
function localizedTargetLabel(doc, target) {
  const raw = targetLabel(doc, target);
  if (raw === "Base") return "基础";
  return raw.replace(/^Character (\d+)/, "角色 $1");
}
...
export function semanticCards(tree, workspaceKey) {
  ...
  if (clothing && !clothing.children.some((node) => node.id === "char_clothing_outfit")) clothing.children.unshift(normalizeSemanticNode({ id: "char_clothing_outfit", label: "服装主体", zh: "服装主体", target: "character", section: "clothing", children: [] }));
...
export class VisualBuilder {
  constructor(options = {}) {
    ...
    // Use a bound arrow so `this.fetchImpl(...)` below calls the global fetch
    // with the correct receiver (matching tag-assistant.js / nsfw-builder.js,
    // which call `fetch` as a plain local reference). Storing the unbound
    // `globalThis.fetch` and calling it as a method throws "Illegal invocation".
    ...
    this.view = { status: this.bridge?.getDocument ? "loading" : "empty", message: this.bridge?.getDocument ? "" : "提示词桥接不可用", error: "", tree: null, semanticState: null, target: this.activeTarget(), path: [], selectedNodeId: "", recommendationStatus: "idle", recommendationError: "", recommendationData: null };
    this._semanticSeq = 0; this._treeSeq = 0; this._recommendationSeq = 0; this._unsubscribe = null; this._lastAddedTag = ""; this._destroyed = false;
  ...
  destroy() { this._destroyed = true; this._semanticSeq += 1; this._treeSeq += 1; this._recommendationSeq += 1; this._unsubscribe?.(); this.root?.removeEventListener("click", this.onClick); this.root?.removeEventListener("change", this.onChange); this.root?.removeEventListener("keydown", this.onKeydown); }
  setActive(value) {
    ...
    if (!this._destroyed) void this.reload();
  }
  ...
  async reloadTree() {
    ++this._treeSeq; const target = this.activeTarget();
    try {
      const data = await this.api("/api/catalog/semantic");
      // The navigation tree is target-independent and immutable for the
      // session.  A concurrent semantic refresh may advance _treeSeq while
      // this request is in flight; dropping the tree in that case leaves the
      // composer stuck in aria-busy="true" even though semantic-state loaded.
      // Only discard a response when the user actually changed workspace.
  ...
  async refreshSemantic() {
    ...
    if (!doc || typeof doc !== "object") { this.view.status = "empty"; this.view.message = "提示词桥接未提供有效的提示词文档"; this.render(); return false; }
    ...
    try {
      ...
      const currentSelectedNodeId = this.view.selectedNodeId; this.render(); if (currentSelectedNodeId) void this.selectNode(currentSelectedNodeId); return true;
    } catch (error) {
      if (seq !== this._semanticSeq || target !== this.activeTarget() || selectedNodeId !== this.view.selectedNodeId) return false;
      this.view.status = "error"; this.view.error = String(error?.message || error); this.render(); return false;
    }
  ...
  addTag(tag, section = "", operation = "add") {
    const target = this.activeTarget();
    if (operation !== "replace") return dispatchAction(this.bridge, buildAddTagAction(tag, target, section));
    const doc = this.bridge?.getDocument?.() || {};
    const slot = this.concreteSlot(this.view.selectedNodeId);
    const replaceTags = slot ? inspectorSelectedEntries(getTargetEntries(doc, target), slot).map((entry) => entry.tag) : [];
    return dispatchAction(this.bridge, { type: "REPLACE_SLOT_TAG", payload: { tag: String(tag || ""), target, section, replaceTags } });
  }
  ...
  handleClick(event) {
    ...
    else if (action === "add") this.addTag(element.dataset.tag, element.dataset.section, element.dataset.operation || "add");
  ...
  render() {
    ...
    if (this.view.status === "error") { this.replaceHtml(`<div class="composer-error" role="alert"><strong>语义目录加载失败</strong><span>${esc(this.view.error)}</span><button type="button" data-action="retry-semantic">重试</button></div>`); return; }
    const doc = this.bridge.getDocument(); const target = this.view.target; const label = localizedTargetLabel(doc, target); const slots = slotsForTarget(this.view.semanticState, target); const completion = completionForSlots(slots); const slotMap = this.slotMap();
    ...
    const cardsHtml = cards.map((node) => { const status = nodeStatus(node, slotMap); const ids = mappedSlotIds(node, slotMap); const evidence = ids.flatMap((id) => slotMap.get(id)?.evidence_tags || []); const auto = ids.some((id) => slotMap.get(id)?.status === "filled_by_auto_preset"); const alternative = ids.find((id) => slotMap.get(id)?.status === "satisfied_by_alternative"); const detail = auto ? `由 ${this.getGenerationConfig()?.positiveTier === "light" ? "轻量" : "标准"}自动提供` : alternative ? (slotMap.get(alternative)?.reason || "由互斥环境分支满足") : evidence.slice(0, 3).join(", ") || node.zh || "尚未设置"; const display = node.zh || node.label; return `<button type="button" class="semantic-node status-${esc(status)}" data-action="node" data-node="${esc(node.id)}" aria-label="${esc(display)}，${esc(STATUS_LABELS[status] || status)}"><span class="status-symbol" aria-hidden="true">${STATUS_SYMBOLS[status]}</span><strong>${esc(display)}</strong><small>${esc(detail)}</small>${node.children.length ? '<span class="drill-hint">查看子项 ›</span>' : ""}</button>`; }).join("");
    const crumbs = [`<button type="button" data-action="crumb" data-depth="0">${esc(label)}</button>`, ...this.view.path.map((node, index) => `<span aria-hidden="true">›</span><button type="button" data-action="crumb" data-depth="${index + 1}">${esc(node.zh || node.label)}</button>`)].join("");
  ...
  renderInspector(doc, label) {
    ...
    if (this.view.recommendationStatus === "error") return `<aside class="semantic-inspector"><h2>${esc(node?.zh || node?.label || slot.label)}</h2><div class="inspector-error" role="alert"><strong>推荐加载失败</strong><span>${esc(this.view.recommendationError)}</span><button type="button" data-action="retry-recommendation">重试</button></div></aside>`;
    ...
    const addButton = (item, fallbackSection = section) => `<button type="button" data-action="add" data-tag="${esc(item.tag)}" data-section="${esc(item.section || fallbackSection)}" data-operation="${esc(item.operation || "add")}">${item.operation === "replace" ? "替换" : "+"} ${esc(item.tag)} → ${esc(label)}</button>`;
    ...
    return `<aside class="semantic-inspector"><h2>${esc(node?.zh || node?.label || slot.label)}</h2><section><h3>已选</h3><div class="selected-chips">${selected || '<span class="muted">尚未添加证据标签</span>'}</div></section><section><h3>当前节点推荐</h3><div class="recommendations">${this.view.recommendationStatus === "loading" ? '<span class="muted">加载中…</span>' : current || '<span class="muted">暂无新的当前节点推荐</span>'}</div></section><section><h3>下一步建议</h3><div class="recommendations next-recommendations">${following || '<span class="muted">暂无下一步建议</span>'}</div></section></aside>`;
  }
}
